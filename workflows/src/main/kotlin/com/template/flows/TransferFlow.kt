...
object TransferFlow{
    ...
    class CoinTransferFlow(var amount: Double, val newOwner: Party) : FlowLogic<SignedTransaction>() {
        ...
        override fun call(): SignedTransaction {
            ...
            val otherPartyMoneyList = otherPartySession.receive<List<StateAndRef<BrunoCoinState>>>().unwrap{ it }

            progressTracker.currentStep = GENERATING_TRANSACTION
            var txBuilder = buildTransaction(listMoneyStateAndRef, notary, otherPartyMoneyList)
...
//            otherPartySession.send(signedTransaction)
        ...
        private fun buildTransaction(listMoneyStateAndRef: List<StateAndRef<BrunoCoinState>>,
                                     notary: Party,
                                     otherPartyMoneyList: List<StateAndRef<BrunoCoinState>>): TransactionBuilder {
            val bCoinOutPutState : BrunoCoinState
            ...
            if(otherPartyMoneyList.isNotEmpty()){

                val gotOtherPartyBrunoCoinState = otherPartyMoneyList.single().state.data
                txBuilder.addInputState(otherPartyMoneyList.single())
                bCoinOutPutState = gotOtherPartyBrunoCoinState.copy(amount = gotOtherPartyBrunoCoinState.amount + amount)
            }else{
                bCoinOutPutState = BrunoCoinState(owner = newOwner, amount = amount)
            }
            txBuilder
    ...
    class TransferResponderFlow(val otherPartySession: FlowSession) : FlowLogic<SignedTransaction>() {
        ...
        override fun call(): SignedTransaction {
            ...
            otherPartySession.send(listMoneyStateAndRef)
